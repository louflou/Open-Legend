...
function getAllValues(){
    ...
    function printPerksFlaws(){
        ...
        for(i = 0; i < perksFlaws.flaws.length;){
            if(perksFlaws.flaws.length > 1){
                flaws += perksFlaws.flaws[i] + ", ";
                i++;
            }else{
                flaws += perksFlaws.flaws[i];
                i++;
    ...
    $("#modall").click(function() {
        ...
        $('#agility.c').html(agility.cost);     $('#agility.d').html(agility.dice);     $('#fortitude.s').html(fortitude.score);     $('#fortitude.c').html(fortitude.cost);     $('#fortitude.d').html(fortitude.dice);
        $('#fortitude.s').html(fortitude.score);     $('#fortitude.c').html(fortitude.cost);     $('#fortitude.d').html(fortitude.dice);
        
        $('#deception.s').html(deception.score);
        $('#deception.c').html(deception.cost);     $('#deception.d').html(deception.dice);     $('#presence.s').html(presence.score);     $('#presence.c').html(presence.cost);     $('#presence.d').html(presence.dice);
        $('#persuasion.s').html(persuasion.score);     $('#persuasion.c').html(persuasion.cost);     $('#persuasion.d').html(persuasion.dice);
        
        $('#learning.s').html(learning.score);
        $('#learning.c').html(learning.cost);     $('#learning.d').html(learning.dice);     $('#logic.s').html(logic.score);     
        $('#logic.c').html(logic.cost);     
        $('#logic.d').html(logic.dice);
        $('#perception.s').html(perception.score);     $('#perception.c').html(perception.cost);     $('#perception.d').html(perception.dice);
        $('#will.s').html(will.score);     
        $('#will.c').html(will.cost);     
        $('#will.d').html(will.dice);
        
        $('#alteration.s').html(alteration.score);
        $('#alteration.c').html(alteration.cost);     $('#alteration.d').html(alteration.dice);     $('#creation.s').html(creation.score);     $('#creation.c').html(creation.cost);     $('#creation.d').html(creation.dice);
        $('#energy.s').html(energy.score);     
        $('#energy.c').html(energy.cost);     
        $('#energy.d').html(energy.dice);
        $('#entropy.s').html(entropy.score);     $('#entropy.c').html(entropy.cost);     $('#entropy.d').html(entropy.dice);
        $('#influence.s').html(influence.score);     $('#influence.c').html(influence.cost);     $('#influence.d').html(influence.dice);
        $('#movement.s').html(movement.score);     $('#movement.c').html(movement.cost);     $('#movement.d').html(movement.dice);
        $('#prescience.s').html(prescience.score);     $('#prescience.c').html(prescience.cost);     $('#prescience.d').html(prescience.dice);
        $('#protection.s').html(protection.score);     $('#protection.c').html(protection.cost);     $('#protection.d').html(protection.dice);
